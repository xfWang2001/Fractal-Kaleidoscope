# 🎨 AI Art & Sensors Interactive Lab (人工智能艺术与传感器交互实验)

欢迎来到 **人工智能艺术与传感器** 课程的交互实验项目仓库！本项目是一系列基于 Web 技术的生成艺术（Generative Art）、AI 视觉识别、以及计算机图形学（WebGL/Three.js）的互动探索合集。

我们使用前端生态（浏览器、摄像头、AI 识别库）搭建了多个无需后端即可实时运行的互动网页。每一个 HTML 文件都是一个独立的艺术实验。
